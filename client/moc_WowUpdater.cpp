/****************************************************************************
** Meta object code from reading C++ file 'WowUpdater.h'
**
** Created: Sun Aug 30 08:32:53 2009
**      by: The Qt Meta Object Compiler version 61 (Qt 4.5.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "WowUpdater.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'WowUpdater.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 61
#error "This file was generated using the moc from 4.5.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_WowUpdater[] = {

 // content:
       2,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   12, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors

 // slots: signature, parameters, type, tag, flags
      12,   11,   11,   11, 0x09,
      28,   11,   11,   11, 0x09,
      67,   11,   11,   11, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_WowUpdater[] = {
    "WowUpdater\0\0slotReadyRead()\0"
    "slotError(QNetworkReply::NetworkError)\0"
    "slotRequestFinished(QNetworkReply*)\0"
};

const QMetaObject WowUpdater::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_WowUpdater,
      qt_meta_data_WowUpdater, 0 }
};

const QMetaObject *WowUpdater::metaObject() const
{
    return &staticMetaObject;
}

void *WowUpdater::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_WowUpdater))
        return static_cast<void*>(const_cast< WowUpdater*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int WowUpdater::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: slotReadyRead(); break;
        case 1: slotError((*reinterpret_cast< QNetworkReply::NetworkError(*)>(_a[1]))); break;
        case 2: slotRequestFinished((*reinterpret_cast< QNetworkReply*(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
